...
for (let i = 0; i < NUMBER_OF_TESTS; i++) {
    ...
    const boxes = Array.from({length: 100}, (_, i) => i + 1).sort((a, b) => 0.5 - Math.random());

    // have the prisoners lost yet?
    let didPrisonersLose = false;

    // each prisoner takes their chance at opening boxes
    for (let prisonerNumber = 1; prisonerNumber <= NUMBER_OF_PRISONERS; prisonerNumber++) {
        // prisoner has not found their number yet
        let hasPrisonerFoundNumber = false;

        // prisoner will start by opening the box # matching their prisoner #
        let boxToOpen = prisonerNumber;

        // prisoner gets to open half of the boxes looking for their own number
        for (let openedBoxCount = 1; openedBoxCount <= (NUMBER_OF_PRISONERS / 2); openedBoxCount++) {
